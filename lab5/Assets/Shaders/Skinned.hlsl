...
//MatrixPalette
cbuffer MATRIX_PALETTE : register(b3) {
	float4x4 skeletonBones[96];
};
...
PS_INPUT VS(VS_INPUT input)
{
	...
	/*Skinning*/
	temp = input.mWeights.x * mul(temp, skeletonBones[input.mBones.x]) +
		input.mWeights.y * mul(temp, skeletonBones[input.mBones.y]) +
		input.mWeights.z * mul(temp, skeletonBones[input.mBones.z]) +
		input.mWeights.w * mul(temp, skeletonBones[input.mBones.w]);
