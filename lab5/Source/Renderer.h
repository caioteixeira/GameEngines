...
public:
	...
	void RenderFrame(std::queue<std::shared_ptr<RenderCommand>> & renderQueue);
	...
	void ExtractComponents(std::queue<std::shared_ptr<RenderCommand>> & updateQueue);
private:
	void Clear();
	void DrawComponents(std::queue<std::shared_ptr<RenderCommand>> & renderQueue);
	void Present();
