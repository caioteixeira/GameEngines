...
void Renderer::RenderFrame(std::queue<std::shared_ptr<RenderCommand>> & renderQueue)
{
	Clear();
	DrawComponents(renderQueue);
	Present();
}
...
void Renderer::ExtractComponents(std::queue<std::shared_ptr<RenderCommand>>& updateQueue)
{
...
void Renderer::DrawComponents(std::queue<std::shared_ptr<RenderCommand>> & renderQueue)
{
	...
	//Run mesh drawing commands
	while(!renderQueue.empty())
	{
		renderQueue.front()->Run(*this);
		renderQueue.pop();
	}
...
bool Renderer::InitShaders()
{
	...
	auto vsPhongLayout = mGraphicsDriver->CreateInputLayout(vsPhongElements, 3, mMeshShaders[EMS_Phong]->GetCompiledVS());
	...
	auto skinnedShader = mGame.GetAssetCache().Load<Shader>("Shaders/Skinned.hlsl");
	if (skinnedShader == nullptr)
		return false;
	mMeshShaders.emplace(EMS_Skinned, skinnedShader);
	InputLayoutElement vsSkinnedElements[] = {
		InputLayoutElement("POSITION", 0, EGF_R32G32B32_Float, 0),
		InputLayoutElement("NORMAL", 0, EGF_R32G32B32_Float, 12),
	...
	auto vsSkinnedLayout = mGraphicsDriver->CreateInputLayout(vsSkinnedElements, 5, mMeshShaders[EMS_Skinned]->GetCompiledVS());
	mInputLayoutCache->RegisterLayout("positionnormalbonesweightstexcoord", vsSkinnedLayout);

	skinnedShader->CreateMatrixPaletteBuffer();
