...
bool CharacterMoveComponent::CheckFootCast(CollisionComponentPtr& outComp, Vector3& outPos)
{
	...
	Vector3 out;
	Vector3 start = outPos;
	Vector3 end = outPos;
	start.z += mFootCastOffset;
	end.z -= mFootCastOffset;

	if (mOwner.GetGame().GetPhysWorld().SegmentCast(mOwner, start, end, outComp, out))
	{
		if(outComp->GetIsWalkable())
		{
			outPos = out;
			return true;
		}
